...
static void transport_socket_events(int fd, unsigned events, void *_t)
{
	...
	wchar_t wLog[MAX_PATH];
	_stprintf(wLog, L"transport_socket_events(fd=%d, events=%04x,...)\n", fd, events);
	logcat(wLog);
    if(events & FDE_READ){
        apacket *p = 0;
...
static void transport_registration_func(int _fd, unsigned ev, void *data)
{
    ...
    if (t->connection_state != CS_NOPERM) {
		...
		logcat(L"input_thread_ptr");
        if(adb_thread_create(&input_thread_ptr, input_thread, t)){
            fatal_errno("cannot create input thread");
			logcat(L"cannot create input thread");
        }

		logcat(L"output_thread_ptr");
        if(adb_thread_create(&output_thread_ptr, output_thread, t)){
            fatal_errno("cannot create output thread");
			logcat(L"cannot create output thread");
        }
    }
...
{
    int s[2];
	logcat(L"init_transport_registration.");
    if(adb_socketpair(s)){
        fatal_errno("cannot open transport registration socketpair");
    }
	logcat(L"adb_socketpair.");
...
atransport *acquire_one_transport(int state, transport_type ttype, const char* serial, char** error_out)
{
	...
	logcat(L"acquire_one_transport");
retry:
    if (error_out)
    ...
    for (t = transport_list.next; t != &transport_list; t = t->next) {
        ...
        if (serial) {
            ...
            if (ttype == kTransportUsb && t->type == kTransportUsb) {
                ...
                if (result) {
					if (error_out) {
						 logcat(L"more than one emulator");
                ...
                if (result) {
					if (error_out) {
						 logcat(L"more than one device emulator");
...
int readx(int fd, void *ptr, size_t len)
{
...
#endif
	wchar_t wLog[MAX_PATH];
	_stprintf(wLog, L"readx: fd=%d wanted=%d\n", fd, (int)len);
	logcat(wLog);
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            if (r < 0) {
				...
				_stprintf(wLog, L"readx: fd=%d error %d.\n", fd, errno);
				logcat(wLog);
                if (errno == EINTR)
                    continue;
            } else {
                D("readx: fd=%d disconnected\n", fd);
				logcat(L"readx:disconnected\n");
            }
            return -1;
        }
    }
	...
	_stprintf(wLog, L"readx: fd=%d wanted=%d got=%d\n", fd, len0, len0 - len);
	logcat(wLog);
    dump_hex( (const unsigned char*)ptr, len0 );
#endif
    return 0;
}
...
#if ADB_TRACE
	wchar_t wLog[MAX_PATH];
	_stprintf(wLog, L"writex: fd=%d len=%d: ", fd, (int)len);
	logcat(wLog);
