...
#if ADB_HOST
    const char *host = NULL; //getenv("ADBHOST");
    if (host) {
...
static void *server_socket_thread(void * arg)
{
	...
	_stprintf(wLog, L"transport: server_socket_thread() starting\n");
	logcat(wLog);
    serverfd = -1;
    for(;;) {
        if(serverfd == -1) {
            ...
            if(serverfd < 0) {
				...
				logcat(L"server: cannot bind socket yet\n");
                adb_sleep_ms(1000);
                continue;
            }
...
/*
    if(adb_thread_create(&thr, func, (void *)port)) {
		wchar_t wLog[MAX_PATH];
		_stprintf(wLog, L"cannot create local socket %s thread", HOST ? L"client" : L"server");
		logcat(wLog);
...
int init_socket_transport(atransport *t, int s, int adb_port, int local)
{
	...
	logcat(L"init_socket_transport");
