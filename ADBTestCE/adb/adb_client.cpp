...
static int switch_socket_transport(int fd)
{
	...
	logcat(L"Switch transport in progress\n");
...
int adb_status(int fd)
{
	...
	logcat(L"adb_status readx");
    if(readx(fd, buf, 4)) {
		logcat(L"protocol fault (no status)");
        strcpy(__adb_error, "protocol fault (no status)");
        return -1;
    }

	logcat(L"adb_status memcmp1");
    if(!memcmp(buf, "OKAY", 4)) {
		logcat(L"adb_status OKAY");
        return 0;
    }

	
	logcat(L"adb_status memcmp2");
    if(memcmp(buf, "FAIL", 4)) {
        sprintf(__adb_error,
                "protocol fault (status %02x %02x %02x %02x?!)",
                buf[0], buf[1], buf[2], buf[3]);
		logcat(L"adb_status FAIL");
        return -1;
    }

	logcat(L"adb_status readx2");
    if(readx(fd, buf, 4)) {
...
int _adb_connect(const char *service)
{
    ...
    int len;
    int fd;
	TCHAR wService[MAX_PATH];
	MultiByteToWideChar(CP_ACP, 0, service, -1, wService, MAX_PATH);
	logcat(L"_adb_connect.");
	logcat(wService);
    len = strlen(service);
    if((len < 1) || (len > 1024)) {
	...
	logcat(L"_adb_connect snprintf.");
    snprintf(tmp, sizeof tmp, "%04x", len);

	logcat(L"_adb_connect socket_loopback_client.");
    fd = socket_loopback_client(__adb_server_port, SOCK_STREAM);
    if(fd < 0) {
        strcpy(__adb_error, "cannot connect to daemon");
		logcat(L"cannot connect to daemon.");
        return -2;
    }

	logcat(L"_adb_connect memcmp.");
    if (memcmp(service,"host",4) != 0 && switch_socket_transport(fd)) {
        return -1;
    }

	logcat(L"_adb_connect writex.");
    if(writex(fd, tmp, 4) || writex(fd, service, len)) {
        strcpy(__adb_error, "write failure during connection");
		logcat(L"write failure during connection.");
        adb_close(fd);
        return -1;
    }
	...
	TCHAR adbLog[MAX_PATH]; 
	_stprintf(adbLog, L"_adb_connect: return fd %d\n", fd);
	logcat(adbLog);
...
int adb_connect(const char *service)
{
	...
	_stprintf(log, L"connect result = %d, service = %s", fd, wService);
	logcat(log);
