...
{
    const char*  p = NULL;//getenv("ADB_TRACE");
    const char*  q;
...
void handle_packet(apacket *p, atransport *t)
{
	...
	_stprintf(wLog, L"handle_packet() %c%c%c%c\n", ((char*) (&(p->msg.command)))[0],
...
{
    asocket *s;
	logcat(L"ss_listener_event_func.");
    if(ev & FDE_READ) {
        ...
        if(s) {
			logcat(L"connect_to_smartsocket.");
...
static int install_listener(const char *local_name, const char *connect_to, atransport* transport)
{
	...
	wchar_t wLog[MAX_PATH];
	wchar_t wlocalName[MAX_PATH];
	wchar_t wConnectTo[MAX_PATH];
	MultiByteToWideChar(CP_ACP, 0, local_name, -1, wlocalName, MAX_PATH);
	MultiByteToWideChar(CP_ACP, 0, connect_to, -1, wConnectTo, MAX_PATH);
	_stprintf(wLog, L"install_listener('%s','%s')\n", wlocalName, wConnectTo);
	logcat(wLog);
    ...
    if((l->local_name = _strdup(local_name)) == 0) goto nomem;
    if((l->connect_to = _strdup(connect_to)) == 0) goto nomem;
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
    ...
    char buf[4096];
	wchar_t wLog[MAX_PATH];
	MultiByteToWideChar(CP_ACP, 0, service, -1, wLog, MAX_PATH);
	wcscat(wLog, L"--handle_host_request--");
	logcat(wLog);
...
//            connect_emulator(host + 4, buffer, sizeof(buffer));
    ...
    if(!strncmp(service,"forward:",8) || !strncmp(service,"killforward:",12)) {
		logcat(L"handle_host_request forward"); 
        ...
        remote = strchr(local,';');
        if(remote == 0) {
			logcat(L"malformed forward spec");
        ...
        *remote++ = 0;
        if((local[0] == 0) || (remote[0] == 0) || (remote[0] == '*')){
			logcat(L"malformed forward spec 1");
            sendfailmsg(reply_fd, "malformed forward spec");
            return 0;
        }
