...
#ifdef _WIN32
    char path[PATH_MAX];
    home = NULL;//getenv("ANDROID_SDK_HOME");
    if (!home) {
		wchar_t wPath[MAX_PATH];
		MultiByteToWideChar(CP_ACP, 0, path, -1, wPath, MAX_PATH);
        //SHGetFolderPath(NULL, CSIDL_PROFILE, NULL, 0, wPath);
		WideCharToMultiByte(CP_ACP,NULL,wPath,-1,path,MAX_PATH,NULL,FALSE);
        home = path;
    }
    format = "%s\\%s";
#else
...
static void get_vendor_keys(struct listnode *list)
{
    ...
    adb_keys_path = NULL;//getenv("ADB_VENDOR_KEYS");
    if (!adb_keys_path)
        return;
