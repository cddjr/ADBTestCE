...
static void local_socket_event_func(int fd, unsigned ev, void *_s)
{
	...
	_stprintf(wLog, L"LS(%d): event_func(fd=%d(==%d), ev=%04x)\n", s->id, s->fd, fd, ev);
	logcat(wLog);
    ...
    */
    ...
    if(ev & FDE_READ){
        ...
        while(avail > 0) {
			...
			_stprintf(wLog, L"LS(%d): post adb_read(fd=%d,...) r=%d (errno=%d) avail=%d\n", s->id, s->fd, r, r<0?errno:0, avail);
			logcat(wLog);
            if(r > 0) {
                avail -= r;
                x += r;
		...
		_stprintf(wLog, L"LS(%d): fd=%d post avail loop. r=%d is_eof=%d forced_eof=%d\n",
        ...
        if((avail == MAX_PAYLOAD) || (s->peer == 0)) {
			...
			_stprintf(wLog, L"LS(%d): fd=%d post peer->enqueue(). r=%d\n", s->id, s->fd, r);
			logcat(wLog);
            if(r < 0) {
...
static int smart_socket_enqueue(asocket *s, apacket *p)
{
    ...
    D("SS(%d): enqueue %d\n", s->id, p->len);
	wchar_t wLog[MAX_PATH];
	_stprintf(wLog, L"SS(%d): enqueue %d\n", s->id, p->len);
	logcat(wLog);
    if(s->pkt_first == 0) {
        s->pkt_first = p;
        s->pkt_last = p;
